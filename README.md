...
graph TD
    ...
    C -->|Relevant docs exist| E["`**Generate**
    LLM answers using
    relevant context`"]

    C -->|No relevant docs| D["`**Web Search**
    Tavily fetches fresh
...
| 3 | **Decide** | If ≥ 1 relevant doc → proceed to **Generate**. If 0 relevant docs → fall back to **Web Search**. |
...
| 6 | **Hallucination Check** | Two checks run: **(a)** Is the answer grounded in the provided facts? **(b)** Does it actually address the question? The result is `passed`, `failed_grounding`, or `failed_relevance`. |
...
| **LLM** | Groq (Llama 3.3 70B Versatile) | Ultra-fast inference for grading & generation |
...
│   ├── ingestion.py              # PDF loading, chunking, embedding & Qdrant upsert
...
|---|---|---|
| **Docker & Docker Compose** | ≥ 20.x / ≥ 2.x | *Required for Option A* |
...
| `top_k` | `int` | ❌ | Number of documents to retrieve (1–20, default: 5) |
...
<p align="center">
  Built with ❤️ using FastAPI · LangGraph · Qdrant · Groq
</p>
]]>
